...
export default function StepBody({ data, errors, onChange }: StepBodyProps) {
   return (
      <div>
         ...
         <p className="text-zinc-400 text-sm mb-8">Estos datos nos permiten calcular tu metabolismo basal.</p>

         <div className="grid grid-cols-2 gap-4 mb-6">
            <div>
               <label htmlFor="weight" className="block text-sm font-medium text-zinc-300 mb-2">Peso (kg)</label>
               <input
                  id="weight"
                  type="number"
                  step="0.1"
               ...
               />
               {errors.weight && <p className="text-red-400 text-sm mt-1.5">{errors.weight}</p>}
            </div>

            <div>
               <label htmlFor="height" className="block text-sm font-medium text-zinc-300 mb-2">Altura (cm)</label>
               <input
                  id="height"
                  type="number"
                  step="1"
               ...
               />
               {errors.height && <p className="text-red-400 text-sm mt-1.5">{errors.height}</p>}
            </div>
         </div>

         <div>
            <label className="block text-sm font-medium text-zinc-300 mb-2">Sexo biológico</label>
            <p className="text-zinc-500 text-xs mb-3">Necesario para el cálculo metabólico, no define tu identidad.</p>
            ...
            {errors.sex && <p className="text-red-400 text-sm mt-2">{errors.sex}</p>}
         </div>
      </div>
   );
}