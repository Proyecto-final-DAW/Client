import type { OnboardingFormData, FormErrors } from "../models/OnboardingFormData";
...
export function validateStep2(data: OnboardingFormData): FormErrors {
   ...
   else if (weight < 30 || weight > 250) errors.weight = "El peso debe estar entre 30 y 250 kg";
   ...
   else if (height < 120 || height > 230) errors.height = "La altura debe estar entre 120 y 230 cm";
...
export function validateStep3(data: OnboardingFormData): FormErrors {
   const errors: FormErrors = {};
   if (!data.activityLevel) errors.activityLevel = "Selecciona tu nivel de actividad";
   return errors;
}
...
export function validateStep(step: number, data: OnboardingFormData): FormErrors {
   switch (step) {
